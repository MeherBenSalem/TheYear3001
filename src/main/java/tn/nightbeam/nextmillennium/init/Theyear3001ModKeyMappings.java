...
import tn.nightbeam.nextmillennium.network.FlyCapsuleGoUpMessage;
import tn.nightbeam.nextmillennium.network.FlyCapsuleGoForwardMessage;
import tn.nightbeam.nextmillennium.network.FlyCapsuleGoDownMessage;
...
public class Theyear3001ModKeyMappings {
	public static final KeyMapping FLY_CAPSULE_GO_UP = new KeyMapping("key.theyear3001.fly_capsule_go_up", GLFW.GLFW_KEY_SPACE, "key.categories.theyear3001") {
		...
		public void setDown(boolean isDown) {
			...
			if (isDownOld != isDown && isDown) {
				Theyear3001Mod.PACKET_HANDLER.sendToServer(new FlyCapsuleGoUpMessage(0, 0));
				...
				Theyear3001Mod.PACKET_HANDLER.sendToServer(new FlyCapsuleGoUpMessage(1, dt));
				FlyCapsuleGoUpMessage.pressAction(Minecraft.getInstance().player, 1, dt);
			}
			isDownOld = isDown;
		}
	};
	public static final KeyMapping FLY_CAPSULE_GO_DOWN = new KeyMapping("key.theyear3001.fly_capsule_go_down", GLFW.GLFW_KEY_LEFT_CONTROL, "key.categories.theyear3001") {
		private boolean isDownOld = false;
		...
		public void setDown(boolean isDown) {
			super.setDown(isDown);
			if (isDownOld != isDown && isDown) {
				Theyear3001Mod.PACKET_HANDLER.sendToServer(new FlyCapsuleGoDownMessage(0, 0));
				...
				Theyear3001Mod.PACKET_HANDLER.sendToServer(new FlyCapsuleGoDownMessage(1, dt));
				FlyCapsuleGoDownMessage.pressAction(Minecraft.getInstance().player, 1, dt);
			}
			isDownOld = isDown;
		}
	};
	public static final KeyMapping FLY_CAPSULE_GO_FORWARD = new KeyMapping("key.theyear3001.fly_capsule_go_forward", GLFW.GLFW_KEY_W, "key.categories.theyear3001") {
		private boolean isDownOld = false;
		...
		public void setDown(boolean isDown) {
			super.setDown(isDown);
			if (isDownOld != isDown && isDown) {
				Theyear3001Mod.PACKET_HANDLER.sendToServer(new FlyCapsuleGoForwardMessage(0, 0));
				...
				Theyear3001Mod.PACKET_HANDLER.sendToServer(new FlyCapsuleGoForwardMessage(1, dt));
