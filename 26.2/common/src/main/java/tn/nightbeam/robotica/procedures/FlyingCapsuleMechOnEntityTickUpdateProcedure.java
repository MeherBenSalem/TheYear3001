...
public class FlyingCapsuleMechOnEntityTickUpdateProcedure {
	public static void execute(Entity entity) {
		...
		if (RoboticaEntityData.getDouble(entity, "VerticalMovement") == 1) {
			entity.setDeltaMovement(new Vec3((entity.getDeltaMovement().x()), 0.3, (entity.getDeltaMovement().z())));
		} else if (RoboticaEntityData.getDouble(entity, "VerticalMovement") == -1) {
			entity.setDeltaMovement(new Vec3((entity.getDeltaMovement().x()), (-0.3), (entity.getDeltaMovement().z())));
		}
		if (!entity.onGround()) {
			if (RoboticaEntityData.getDouble(entity, "ForwardMovement") == 1) {
